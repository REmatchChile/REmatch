...
def main():
    ...
    for query in TOT_QUERIES:
        writeInCell(
            ...
            get_rgx(
                "{0}/exp/benchmark/{1}/exp0{2}/rematch.rgx".format(
                    HOME_DIR, DATASET, query
                )
            ),
        )
        writeInCell(
            ...
            get_rgx(
                "{0}/exp/benchmark/{1}/exp0{2}/perl.rgx".format(
                    HOME_DIR, DATASET, query
                )
            ),
        )
        ...
        for row in TOT_ROWS:
            for col in TOT_COLS:
                ...
                writeInCell(
                    ...
                    spreadsheet_id,
                    "Query {0}!{1}{2}".format(
                        query, chr(ord(col) + 2 * len(BINARIES)), row + 2
                    ),
                    nout,
                )
                ...
                writeInCell(
                    ...
                    spreadsheet_id,
                    "Query {0}!{1}{2}".format(
                        query, chr(ord(col) + len(BINARIES)), row + 2
                    ),
                    mem,
                )
