...
CONFIG_FILE_PATH = pth.join(HERE, "config.json")
...
def automata_stats(doc_path: str, rgx_path: str):
	command = [f"{HOME_DIR}/build/Release/bin/rematch",
             "--searching",
            #  "--macrodfa",
             "--mode=benchmark",
             f"--document={doc_path}",
...
def run_experiments():
	...
	for dataset in sorted(next(os.walk(exps_path, followlinks=True))[1]):
		for experiment in sorted(map(lambda x: x[0], filter(lambda x: bool(x[2]), os.walk(os.path.join(exps_path, dataset))))):
			...
			for bin, bin_data in BINARIES.items():
				...
				time, memory, noutputs = run_bench(bin, doc_path, rgx_path, NEXP)
	...
	return time_df, mem_df, out_df

if __name__ == '__main__':
	T, M, O = run_experiments()