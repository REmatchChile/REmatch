...
ECSNode* create_extend_node(ECS* ecs, int variable_annotations, int document_position) {
  ECSNode* base_node = create_bottom_node(ecs);
  ECSNode* output_node =
      ecs->create_extend_node(base_node, variable_annotations, document_position);
...
ECSNode* create_mempool_sized_binary_tree_of_union_nodes(ECS* ecs) {
  ...
  for (int i = starting_point_of_bottom_nodes; i < (int)MEMORY_POOL_STARTING_SIZE; i++) {
  ...
  while (amount_of_nodes_of_previous_layer > 1) {
    ...
    int starting_point_of_current_layer =
        starting_point_of_previous_layer - amount_of_nodes_of_current_layer;
    for (int i = starting_point_of_current_layer; i < starting_point_of_previous_layer; i++) {
