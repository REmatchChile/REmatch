...
std::vector<ECSNode*> create_numbered_label_nodes_that_are_connected(NodeManager* node_manager,
                                                                     int amount_of_nodes_to_alloc,
                                                                     int starting_label) {
  ...
  for (int i = 1; i < amount_of_nodes_to_alloc; i++) {
    ECSNode* new_node =
        node_manager->alloc(ECSNodeType::kLabel, last_node, nullptr, 0, i + starting_label);
...
TEST_CASE("Garbage collector doesn't recycle nodes that are being used") {
  ...
  std::vector<ECSNode*> nodes =
      create_numbered_label_nodes_that_are_connected(&node_manager, MEMORY_POOL_STARTING_SIZE, 0);
...
TEST_CASE(
  ...
  std::vector<ECSNode*> original_nodes =
      create_numbered_label_nodes_that_are_connected(&node_manager, MEMORY_POOL_STARTING_SIZE, 0);
  node_manager.decrease_ref_count(original_nodes[MEMORY_POOL_STARTING_SIZE - 1]);

  create_numbered_label_nodes_that_are_connected(&node_manager, MEMORY_POOL_STARTING_SIZE,
  ...
  for (int i = 0; i < (int)MEMORY_POOL_STARTING_SIZE; i++) {
    REQUIRE((size_t)original_nodes[MEMORY_POOL_STARTING_SIZE - i - 1]->document_position ==
...
TEST_CASE("The duplication of memory is working correctly") {
  ...
  REQUIRE(node_manager.amount_of_nodes_allocated() == MEMORY_POOL_STARTING_SIZE);
  int amount_of_nodes_to_allocate = MEMORY_POOL_STARTING_SIZE;
  for (int memory_duplication_time = 1; memory_duplication_time < 5; memory_duplication_time++) {
    ...
    REQUIRE(amount_of_nodes_that_n_minipool_should_allocate(memory_duplication_time) ==
