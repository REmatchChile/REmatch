...
TEST_CASE("One union returns two mappings") {
  ECS ecs = ECS();
  int depth = GENERATE(0, 3, 7, 31);
  ECSNode* union_node = ecs.create_union_node(create_linked_list_node_of_depth(&ecs, depth),
...
void add_oppening_annotation_to_variable(std::bitset<64>& variable_markers, int variable) {
  variable_markers[variable * 2] = true;
}

void add_closing_annotation_to_variable(std::bitset<64>& variable_markers, int variable) {
...
ECSNode* extend_current_node_with_annotation(ECS* ecs, ECSNode* current_node,
  ...
  ECSNode* new_left_node =
      ecs->create_extend_node(current_node, variable_markers, last_document_position--);
  ECSNode* new_right_node =
      ecs->create_extend_node(current_node, variable_markers, last_document_position--);
  ECSNode* new_union_node = ecs->create_union_node(new_left_node, new_right_node);
  return new_union_node;
}

ECSNode* extend_current_node_with_closing_hexagon(ECS* ecs, ECSNode* current_node,
                                                  int variable_number,
                                                  int& last_document_position) {
  std::bitset<64> variable_markers;
  add_closing_annotation_to_variable(variable_markers, variable_number);
  return extend_current_node_with_annotation(ecs, current_node, variable_markers,
                                             last_document_position);
}

ECSNode* extend_current_node_with_opening_hexagon(ECS* ecs, ECSNode* current_node,
                                                  int variable_number,
                                                  int& last_document_position) {
  std::bitset<64> variable_markers;
...
ECSNode* create_complex_ecs_structure_1(ECS* ecs, int amount_of_variables) {
  ...
  for (int i = 0; i < amount_of_variables; i++)
    current_node =
        extend_current_node_with_closing_hexagon(ecs, current_node, i, last_document_position_used);
  for (int i = 0; i < amount_of_variables; i++)
    current_node =
        extend_current_node_with_opening_hexagon(ecs, current_node, i, last_document_position_used);
  return current_node;
}
...
ECSNode* create_single_variable_output_node_with_position(ECS* ecs, int document_position) {
  std::bitset<64> variable_markers;
  variable_markers[0] = true;
  variable_markers[1] = true;
  ECSNode* output =
      ecs->create_extend_node(ecs->create_bottom_node(), variable_markers, document_position);
  REQUIRE(output != nullptr);
  return output;
}

ECSNode* create_binary_tree_of_union_nodes_with_one_variable_and_empty_captures(ECS* ecs,
                                                                                int depth) {
  ...
  for (int layer_number = depth - 1; layer_number >= 0; layer_number--) {
    ...
    for (int i = 0; i < (1 << layer_number); i++) {
      current_layer[i] = ecs->create_union_node(last_layer[i * 2], last_layer[i * 2 + 1]);
    }
    delete[] last_layer;
    last_layer = current_layer;
  }
  auto root = last_layer[0];
  delete[] last_layer;
  return root;
}
...
TEST_CASE(
  ...
  ECSNode* root =
      create_binary_tree_of_union_nodes_with_one_variable_and_empty_captures(&ecs, depth);
  Enumerator enumerator = Enumerator();
  enumerator.add_node(root);

  std::vector<int> spans = std::vector<int>();
  while (enumerator.has_next()) {
    std::vector<Span> spans_matched = enumerator.next()->get_spans_of_variable_id(0);
