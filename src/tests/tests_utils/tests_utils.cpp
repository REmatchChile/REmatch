...
#include "mediator/multi_finditer_mediator/multi_finditer_mediator.hpp"
...
void run_algorithm_test(const std::string& query, const std::string& document_,
  ...
  const auto* result_mapping = algorithm.next();
...
void run_match_iterator_test(const std::string& query_, const std::string& document,
                             std::vector<std::map<std::string, Span>> expected_matches) {

  auto query = reql(query_);
  auto match_generator = query.finditer(document);
  std::vector<std::map<std::string, Span>> actual_matches;
  ...
  INFO(info_os.str());
  REQUIRE(actual_matches == expected_matches);
}
...
std::string create_document_with_repeated_string(const std::string& str, int times) {
  std::ostringstream os;
...
bool extended_va_has_only_read_captures_transitions(ExtendedVA const& extended_va) {
  for (auto& state : extended_va.states) {
    if (!(state->filters.empty() && state->backward_filters.empty() && state->captures.empty() &&
          state->backward_captures.empty()))
      return false;
  }
...
std::bitset<64> get_markers(const std::string& bits) {
  ...
  variable_markers[size - 1] = true;
  variable_markers[size - 2] = true;
