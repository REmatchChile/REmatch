...
std::unique_ptr<MultiMatch> MultiQuery::findone(const std::string& document_) {
  ...
  return std::make_unique<MultiMatchStandard>(std::move(mapping), query_data_->variable_catalog,
                                              document);
}

std::unique_ptr<MultiMatch> MultiQuery::findone(Reader* reader) {
  ...
  return std::make_unique<SMultiMatch>(std::move(mapping), query_data_->variable_catalog, stream);
}

std::vector<std::unique_ptr<MultiMatch>> MultiQuery::findmany(const std::string& document,
                                                              uint_fast32_t limit) {
...
std::vector<std::unique_ptr<MultiMatch>> MultiQuery::findmany(Reader* reader, uint_fast32_t limit) {
  std::vector<std::unique_ptr<MultiMatch>> res;
...
std::vector<std::unique_ptr<MultiMatch>> MultiQuery::findall(const std::string& document) {
  std::vector<std::unique_ptr<MultiMatch>> res;
...
std::vector<std::unique_ptr<MultiMatch>> MultiQuery::findall(Reader* reader) {
  std::vector<std::unique_ptr<MultiMatch>> res;
...
SMultiMatchGenerator MultiQuery::finditer(Reader* reader) {
  auto stream = std::make_shared<Stream>(reader, buffer_size);
