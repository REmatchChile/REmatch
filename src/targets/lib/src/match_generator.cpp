...
MatchGenerator::iterator::iterator(std::unique_ptr<FinditerMediator> mediator_,
...
MatchGenerator::MatchGenerator(std::shared_ptr<QueryData> query_data, const std::string& document,
...
MatchGenerator::iterator MatchGenerator::begin() const {
  auto mediator = std::make_unique<FinditerMediator>(
      *query_data, document, flags, max_mempool_duplications, max_deterministic_states);
