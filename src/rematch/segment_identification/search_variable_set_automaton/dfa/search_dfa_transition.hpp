...
struct DCapture {
  ...
  DCapture(std::bitset<64> S, SearchDFAState* q): S(S), next(q) {}
};

using SearchDFAStates = std::vector<SearchDFAState*>;
...
struct Transition {
  ...
  Transition(SearchDFAState* state): type_(Type::kDirect), direct_(state) {}
  ...
  void add_direct(SearchDFAState* state) {
    switch (type_) {
      case Type::kEmpty:
