...
#include "utils/document/document.hpp"
 ...
 public:
  ...
  StrDocument(const char* str)
	...
	struct char_iterator {
		...
		char_iterator(const char *d, size_t s)
				:	data_(d), size_(s), current_(d) {}

		const char& operator*() const { return *current_; }

		char_iterator& operator++() {current_++; return *this;}
	...
	struct line_iterator {
		...
		line_iterator& operator++() { next_line(); return *this; }
	...
	CharIterator begin() const {return CharIterator(std::move(char_iterator(data_, size_)));}
  EndCharIterator end() const {return EndCharIterator();}

	LineIterator line_begin() const {return LineIterator(line_iterator(data_, size_));}
