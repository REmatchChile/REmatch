...
public:
  ...
  internal::Node *alloc(std::bitset<32> S, int i, internal::Node *head,
                        internal::Node *tail) {
...
public:
  ...
  MemManager()
      : current(new MiniPool(STARTING_SIZE)), freeHead(nullptr), totElements(0),
        totArenas(1), totElementsReused(0), tot_adyacent_extends_(0),
        tot_head_extends_(0) {}
  ...
  void reset() {
    ...
    current = new MiniPool(STARTING_SIZE), freeHead = nullptr;
    totElements = 0;
    totArenas = 1;
    totElementsReused = 0;
  }
