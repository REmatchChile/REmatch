...
LineEvaluator::LineEvaluator(RegEx &rgx, std::shared_ptr<Document> d)
    : rgx_(rgx),
      enumerator_(rgx_),
      text_(d),
      i_pos_(0) {
...
Match_ptr LineEvaluator::next() {
  ...
  if(current_line_ != text_->line_end()) {
    for(; current_line_ != text_->line_end(); ++current_line_) {
      ...
      for(auto it = (*current_line_).begin(); it != (*current_line_).end(); ++it) {
        a = *it;
...
bool LineEvaluator::match(const std::string &s) {
  char a;
...
void LineEvaluator::reading(char a, int64_t pos) {
...
inline void LineEvaluator::pass_current_outputs() {
