...
ExtendedDetVA::ExtendedDetVA(ExtendedVA& extended_va, uint_fast32_t max_deterministic_states)
    : extended_va_(extended_va), dfa_states_checker_(max_deterministic_states) {
  state_manager = std::make_unique<DefaultStateManager>(extended_va.size());
...
std::vector<CaptureSubsetPair> ExtendedDetVA::convert_to_dfa_states(
  ...
  for (auto& bitset_subset_pair : captures_subset_map) {
    ExtendedDetVAState* next_state = state_manager->get_state_from_subset(bitset_subset_pair.second);
