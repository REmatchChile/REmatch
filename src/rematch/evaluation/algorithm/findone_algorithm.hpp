...
class FindoneAlgorithm : public AlgorithmClass {
 public:
  FindoneAlgorithm(
      ExtendedVA& extended_va, std::shared_ptr<Document> document, Flags flags = Flags(),
 ...
 private:
  void evaluate();
  void update_output_nodes(ExtendedDetVAState*& next_state, ECSNode*& next_node) override;
  void enumerate();
};
