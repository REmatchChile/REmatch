...
// Charclass visitor
rematch::CharClass &charclass_visitor :: operator()(automata::special_set const &ss) const
{
    rematch::CharClass *chrcls = new rematch::CharClass(ss, true);
    return *chrcls;
}


rematch::CharClass &charclass_visitor :: operator()(automata::charset const &cs) const
{
    bool negated;
    std::set<rematch::CharClass::range> ranges;
    ...
    for (automata::charset::element const &element : cs.elements)
    {
        ...
        if (element.which() == 0)
        {
            singles.insert(boost::get<char>(element));
        }
