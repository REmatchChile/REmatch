...
std::vector<std::string> get_test_folders(std::string dir_path) {
  // Returns all the folders that start with "test" inside the given folder
  // (POSIX only?)
...
void check_spanners(rematch::RegEx &rgx, std::string document_file,
                    std::string spanners_file) {
  ...
  rematch::MatchIterator m_iter = rgx.find_iter(document);
  ...
  std::set_difference(
      real_results.begin(), real_results.end(), expected_results.begin(),
  ...
  std::string additional_values_stream =
      boost::algorithm::join(additional_values_flat, " & ");
  std::string missing_values_stream =
      boost::algorithm::join(missing_values_flat, " & ");

  BOOST_TEST_INFO("Additional values yielded by the evaluator: "
                  << additional_values_stream);
  BOOST_TEST_INFO(
      "Missing values yielded by the evaluator: " << missing_values_stream);
  BOOST_TEST_REQUIRE(real_results == expected_results,
                     "Expected spanners do not match with given spanners");
}

/*
  TEST CASE 1:
    - Name: test_input_automata_file
    - Description: load the automata from automata.txt, process document.txt and
  check if given spanners are the same than those in spanners.txt
    - Tests folder: /tests/test_input_automata_file/
    - Test specific folder: every folder that starts with "test" inside de tests
  folder
    - Test specific folder files: automata.txt, document.txt, spanners.txt
*/

// const std::string test_input_automata_file_folder =
// "tests/test_input_automata_file"; const std::vector<std::string>
// test_input_automata = get_test_folders(test_input_automata_file_folder);
...
//     test_folder)
// {
//     std::string current_test_folder = test_input_automata_file_folder + "/" +
//     test_folder + "/"; BOOST_TEST_MESSAGE("RUNNING TEST " +
//     current_test_folder + "\n");
...
/*
  TEST CASE 2:
    ...
    - Description: load the automata from regex.txt, process document.txt and
  check if given spanners are the same than those in spanners.txt
...
const std::vector<std::string> test_input_regex =
    get_test_folders(test_input_regex_file_folder);

BOOST_DATA_TEST_CASE(test_input_regex_file, data::make(test_input_regex),
                     test_folder) {
  std::string current_test_folder =
      test_input_regex_file_folder + "/" + test_folder + "/";
