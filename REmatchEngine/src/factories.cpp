...
void VariableFactory :: fillPartialMapping(std::map<std::string, std::pair<size_t,size_t>> &partialMapping , std::bitset<32> code, int i) {
	unsigned int aux;
	for(size_t j=0; j < 16; j++) {
		if(code[2*j]) {
			partialMapping[getVarName(j)].first = i;
		}
		if(code[2*j+1]) {

			partialMapping[getVarName(j)].second = i;
		}
	}
