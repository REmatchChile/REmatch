...
LVAState *file_to_automata :: get_state(std::string state_name) 
{
    if (!states_map.count(state_name))
    {
        ...
        automata->numStates = automata->states.size();
    }
...
{
    if (!automata->vFact->isMember(variable.name)) 
    {
        automata->vFact->addVar(variable.name);
    }
    if (variable.is_opening) 
    {
        return automata->vFact->getOpenCode(variable.name);
    }
    return automata->vFact->getCloseCode(variable.name);
} 
...
int file_to_automata :: get_filter_code(automata::charclass const &automata_charclass) {
    ...
    if (!automata->fFact->isMember(charclass)) 
    {
        automata->fFact->addFilter(charclass);
    }
    return automata->fFact->getCode(charclass);
}
